...
export default function GameBoard({ room, currentTurn, myPlayerNumber, hand, onDraw, status }: GameBoardProps) {
  ...
  return (
    <div className="relative w-full h-screen bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-slate-900 via-black to-black flex flex-col items-center justify-between p-4 overflow-hidden">
      ...
      {/* DECK E MÃO (BASE) */}
      <div className="w-full px-1 max-w-4xl flex flex-col-reverse md:flex-row items-end md:items-center justify-center  gap-8 relative pb-4">
        
        {/* MÃO DO JOGADOR */}
        <div className="flex  overflow-x-scroll md:overflow-hidden items-end md:items-center -space-x-4 bg-black/40 p-4 rounded-xl border border-white/5 backdrop-blur-sm">
          ...
          <div className="relative z-20 mx-4 mt-2">
            <div className={`w-28 h-40 bg-slate-800 rounded-md border-2 border-yellow-500/50 overflow-hidden shadow-[0_0_25px_rgba(234,179,8,0.2)] ${triggerAnim ? 'animate-draw' : ''}`}>
               {lastDrawnCard ? (
                 <img src={cardMap[lastDrawnCard] || "/back.jpg"} className="w-full h-full mb-20 object-cover" alt="Drawn Card" />
               ) : (
                 <div className="w-full h-full flex items-center justify-center text-slate-300 text-[8px] text-center p-2">PUXE UMA CARTA</div>
               )}
            </div>
